...
c = requests.session()
bot_username = '@Ershkalibot'
bot_username2 = '@Ershkalibot'
bot_username3 = '@Ershkalibot'
bot_username4 = '@EEObot'
...
async def Hussein(event):
    if event.message.message.startswith("تجميع ارشقلي") and str(event.sender_id) in ConsoleJoker:
        await event.reply("**᯽︙سيتم تجميع النقاط من بوت هيلاس , قبل كل شي تأكد من انك قمت بلانظمام الى القنوات الاشتراك الاجباري للبوت لعدم حدوث اخطاء**")
        channel_entity = await l313l.get_entity(bot_username2)
        await l313l.send_message('@Ershkalibot', '/start')
        await asyncio.sleep(2)
        msg0 = await l313l.get_messages('@Ershkalibot', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(2)
        msg1 = await l313l.get_messages('@Ershkalibot', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await l313l(ImportChatInviteRequest(bott))
                msg2 = await l313l.get_messages('@Ershkalibot', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
...
async def _(event):
 ...
 #   else:
   #     await event.edit("يجب الدفع لاستعمال هذا الامر !")
@l313l.on(admin_cmd(pattern="(تجميع المليار|تجميع المليار)"))
async def _(event):
    if JoKeRUB[0] == "yes":
