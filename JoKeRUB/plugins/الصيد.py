...
hrrrbChecler_cmd = (
    "• اوامـر الصيـد والتثبيت هيلاس سورس • \n\n"
...
async def _(event):
    ...
    try:
        ...
        ch = await l313l(
            functions.channels.CreateChannelRequest(
                title="⎉ تثبيت HELLAS ⎉",
                about=f"تم تثبيت اليـوزر بواسطـة سـورس هيلاس - @HELLASUserBot | {rob} ",
            )
        )
        try:
            ch = ch.updates[1].channel_id
...
async def _(event):
    ...
    bmodels = True
    rrrnm = "⎉ تثبيت HELLAS  ⎉"
    rrrby = "تم تثبيت اليـوزر بواسطـة سـورس 𝐇𝐞𝐥𝐥𝐚𝐬 - @HELLASUserBot "
    while bmodels:
